...
struct Conflict {
  ...
  Conflict()
      : time(0), agent1(0), agent2(0), type(Vertex), agent1_start(),
        agent1_end(), agent2_start(), agent2_end() {}
  ...
  friend std::ostream &operator<<(std::ostream &os, const Conflict &c) {
    ...
    case Vertex:
      return os << c.time << ": Vertex(" << c.agent1_start.x << ","
                << c.agent1_start.y << ") <-> (" << c.agent2_start.x << ","
                << c.agent2_start.y << ")";
    case Edge:
      return os << c.time << ": Edge((" << c.agent1_start.x << ","
                << c.agent1_start.y << ") -> (" << c.agent1_end.x << ","
                << c.agent1_end.y << ")) <-> ((" << c.agent2_start.x << ","
                << c.agent2_start.y << ") -> (" << c.agent2_end.x << ","
                << c.agent2_end.y << "))";
    }
    return os;
  }
...
private:
  ...
  bool transitionsWithinClearance(const State &state1a, const State &state1b,
    ...
    return segmentDistanceSquared(
               ...
               static_cast<double>(state2b.x), static_cast<double>(state2b.y)) <=
